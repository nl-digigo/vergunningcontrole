...
| 4 | Wie is repo-beheerder? | Eén persoon met mandaat over toegang, structuur en releases. Zie §4. | Projectteam |
| 5 | Statusmodel | `Werk in uitvoering` → `Gedeeld` → `Definitief` (7 sep afgesproken) koppelen aan de ReSpec `spec-status` in de config. | Lex |
...
De strategie hierboven valt of staat bij vier rollen die nu nog niemand formeel heeft.

### 4.1 De rollen

| Rol | Voorstel | Mandaat en taken |
|---|---|---|
| **Opdrachtgever / eindredactie** | **Jeroen** | Bepaalt wat wanneer naar buiten gaat en met welke status. Hakt knopen door over structuur en scope. Lijn naar de stuurgroep en naar digiGO (Henk Hutink) over huisstijl en licentiebeleid. |
| **Repo-beheerder** | **Lex** | De rol waar Lex in het consult om vroeg. Beheert toegang, bewaakt mapstructuur, naamgeving en de index-bestanden, zet tags bij releases, bewaakt dat er geen kapotte ReSpec in main komt. Eén persoon, anders verzandt het. |
| **Technisch inrichter** | **Peter** | ReSpec-config, `spec-status`, GitHub Pages, samenvoegen van de VNG-repo met de digiGO-repo, later het inrichten van de consultatie-omgeving met commentaar in de kantlijn. Verleent ook GitHub-toegang aan het team. |
| **Documenteigenaar** | per document, zie 4.2 | Eén naam per document. Schrijft, verwerkt reviews, meldt aan de repo-beheerder wanneer het document release-waardig is. |
...
| **Leveranciersschakel** | **Siham** | Brengt de acceptatie-branch en de pseudocode naar de leveranciers, haalt hun bevindingen terug als issues. Heeft de GitHub-inrichting opgezet en houdt de lijn met Rolf en Martijn. |
| **Vraagbaak ReSpec** | **Elisabeth de Vries (digiGO)** | Collegiaal, per mail, geen projectrol. Zij gaf aan dat ze dit als onderdeel van haar werk als productmanager ziet en dat een halfuurtje vragen altijd kan. Gebruik dat gericht — met concrete vragen, niet met een open sessie. |
...
| De 17 checks — beschrijvend | Gerlof | Lex, Siham, koplopers |
| Technische specificatie / pseudocode | Peter | Lex, Siham, leveranciers |
| ILS Omgevingsvergunning + IDS | Lex | Peter, Gerlof |
| Geodata-eisen | Gerlof | Lex |
| Begrippenlijst (dwars door alles) | Lex | iedereen |

Een documenteigenaar is geen exclusieve schrijver — iedereen mag in elk bestand — maar is wel degene die "af" mag zeggen en die de samenhang bewaakt.

### 4.3 Samenwerkingsafspraken
...
6. Structuurwijzigingen lopen via de repo-beheerder — niet zelf hoofdstukken hernummeren, zoals vandaag gebeurde met laag 1.
...
| Deze week | Mapstructuur opnieuw inrichten: `docs`, `media`, één bestand per check, index-bestanden | Lex |
| Deze week | Afbeeldingen uit de markdown halen en naar `media` verplaatsen, relatieve paden zetten | Lex (Gerlofs 3 issues) |
| Deze week | `spec-status` op Werk in uitvoering, README met naamgeving, licentie en werkafspraken | Lex |
| Week van 22 sep | Begrippenlijst v0 opzetten uit bestaande digiGO-lijsten | Lex |
...
| **30 sep** | **Eerste publicatie: het beschrijvende checks-document, met tag en release** | Jeroen + Lex |
...
**De installatiedrempel.** Drie van de zes teamleden moeten nog beginnen met VS Code en GitHub Desktop, en het is niet triviaal. Als dit blijft hangen, schrijft straks alleen Lex. Plan er een gezamenlijk half uur voor in plaats van het aan ieders agenda over te laten.

**Kapotte ReSpec zonder dat iemand het merkt.** Een enkel leesteken kan een heel hoofdstuk uit de publicatie laten verdwijnen, zonder foutmelding op de site. Daarom de regel over lokaal renderen vóór het committen, en daarom een repo-beheerder die de gepubliceerde pagina periodiek naloopt.
