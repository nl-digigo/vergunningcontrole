# BIM-modellen beoordelen — in korte filmpjes

Je krijgt bij een vergunningsaanvraag steeds vaker een BIM-model aangeleverd, als IFC-bestand. Op deze pagina staat een reeks korte filmpjes die je stap voor stap laat zien wat je daarmee kunt.

Geen voorkennis nodig. Geen dure software nodig. Elk filmpje duurt twee tot vier minuten en behandelt één ding.

**Begin bij het begin en werk de reeks van boven naar beneden af.** Weet je al wat een IFC is? Sla de eerste filmpjes gerust over en spring naar het onderdeel dat je nodig hebt.

Door alle filmpjes loopt dezelfde casus: één vergunningsaanvraag, die we van binnenkomst tot controle volgen.

<!--
INSTRUCTIE VOOR DE BEHEERDER — verwijder dit blok voor publicatie.
Vervang per filmpje de placeholder (FILMPJE-1, FILMPJE-2, ...) door de YouTube-ID:
dat is het stukje achter v= in de URL. Zoek-en-vervang in dit bestand.
Draai je deze pagina via GitHub Pages? Dan kun je in elk blok de uitgecommentarieerde
iframe-variant gebruiken in plaats van de thumbnail. Op github.com zelf werkt alleen
de thumbnail — iframes worden daar weggefilterd.
-->

---

## De basis

### 1. Wat is een BIM-model, en wat is een IFC?

<!-- ▼ VIDEOBLOK — vervang FILMPJE-1 door de YouTube-ID ▼ -->

[![Filmpje 1 — Wat is een BIM-model, en wat is een IFC?](https://img.youtube.com/vi/FILMPJE-1/hqdefault.jpg)](https://youtu.be/pIr8xzqIrVg))

<!-- GitHub Pages-variant:
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/FILMPJE-1" title="Filmpje 1 — Wat is een BIM-model, en wat is een IFC?" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
-->

<!-- ▲ EINDE VIDEOBLOK ▲ -->

Je krijgt straks geen pdf meer, maar een IFC-bestand. In twee minuten leggen we uit wat dat is en waar het vandaan komt.

**2 minuten** · Start hier als je nog nooit met BIM hebt gewerkt.

---

### 2. Er komt een aanvraag binnen. Wat doe je?

<!-- ▼ VIDEOBLOK — vervang FILMPJE-2 door de YouTube-ID ▼ -->

[![Filmpje 2 — Er komt een aanvraag binnen](https://img.youtube.com/vi/FILMPJE-2/hqdefault.jpg)](https://www.youtube.com/watch?v=FILMPJE-2)

<!-- GitHub Pages-variant:
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/FILMPJE-2" title="Filmpje 2 — Er komt een aanvraag binnen" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
-->

<!-- ▲ EINDE VIDEOBLOK ▲ -->

Een aanvraag met een IFC erbij. We laten zien hoe je het bestand opslaat en waarmee je het openmaakt.

**2 minuten** · Geen voorkennis nodig.

---

### 3. Hoe werkt een BIM-viewer?

<!-- ▼ VIDEOBLOK — vervang FILMPJE-3 door de YouTube-ID ▼ -->

[![Filmpje 3 — Hoe werkt een BIM-viewer?](https://img.youtube.com/vi/FILMPJE-3/hqdefault.jpg)](https://www.youtube.com/watch?v=FILMPJE-3)

<!-- GitHub Pages-variant:
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/FILMPJE-3" title="Filmpje 3 — Hoe werkt een BIM-viewer?" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
-->

<!-- ▲ EINDE VIDEOBLOK ▲ -->

Er zijn veel BIM-viewers en veel ervan zijn gratis. We laten zien hoe ze werken — en waarom ze allemáál ongeveer hetzelfde werken, zodat het niet uitmaakt welke jouw gemeente kiest.

**3 à 4 minuten** · Kies er eentje uit de lijst met viewers en kijk mee.

---

## Aan de slag met het model

### 4. Filteren: alleen zien wat je nodig hebt

<!-- ▼ VIDEOBLOK — vervang FILMPJE-4 door de YouTube-ID ▼ -->

[![Filmpje 4 — Filteren](https://img.youtube.com/vi/FILMPJE-4/hqdefault.jpg)](https://www.youtube.com/watch?v=FILMPJE-4)

<!-- GitHub Pages-variant:
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/FILMPJE-4" title="Filmpje 4 — Filteren" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
-->

<!-- ▲ EINDE VIDEOBLOK ▲ -->

Voordat je iets kunt controleren, moet je het kunnen vinden. Hoe krijg je alleen de ramen, alleen de wanden of alleen de gevel in beeld?

**3 minuten** · Dit is de stap die je bij elke controle opnieuw zet — sla dit filmpje niet over.

---

## Het gebouw en de omgeving

### 5. Wat is geo-informatie?

<!-- ▼ VIDEOBLOK — vervang FILMPJE-5 door de YouTube-ID ▼ -->

[![Filmpje 5 — Wat is geo-informatie?](https://img.youtube.com/vi/FILMPJE-5/hqdefault.jpg)](https://www.youtube.com/watch?v=FILMPJE-5)

<!-- GitHub Pages-variant:
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/FILMPJE-5" title="Filmpje 5 — Wat is geo-informatie?" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
-->

<!-- ▲ EINDE VIDEOBLOK ▲ -->

De kaartlagen van heel Nederland staan gratis online bij PDOK. Wat staat daar, en hoe leg je ze over elkaar heen?

**2 à 3 minuten** · Kijk dit voordat je aan filmpje 6 begint.

---

### 6a. Het model op de kaart

<!-- ▼ VIDEOBLOK — vervang FILMPJE-6A door de YouTube-ID ▼ -->

[![Filmpje 6a — Het model op de kaart](https://img.youtube.com/vi/FILMPJE-6A/hqdefault.jpg)](https://www.youtube.com/watch?v=FILMPJE-6A)

<!-- GitHub Pages-variant:
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/FILMPJE-6A" title="Filmpje 6a — Het model op de kaart" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
-->

<!-- ▲ EINDE VIDEOBLOK ▲ -->

Het model van de aanvraag, op de kaart, op het echte perceel. We doen het voor in QGIS — maar de stappen zijn in elke GIS-omgeving hetzelfde.

**3 à 4 minuten** · Kijk eerst filmpje 5.

---

### 6b. Andersom: de kaart in je BIM-viewer

<!-- ▼ VIDEOBLOK — vervang FILMPJE-6B door de YouTube-ID ▼ -->

[![Filmpje 6b — De kaart in je BIM-viewer](https://img.youtube.com/vi/FILMPJE-6B/hqdefault.jpg)](https://www.youtube.com/watch?v=FILMPJE-6B)

<!-- GitHub Pages-variant:
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/FILMPJE-6B" title="Filmpje 6b — De kaart in je BIM-viewer" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
-->

<!-- ▲ EINDE VIDEOBLOK ▲ -->

Je kunt het gebouw naar de kaart brengen, of de kaart naar het gebouw. Deze route gaat de andere kant op — vaak sneller als je geen GIS-omgeving gebruikt.

**3 minuten** · Optioneel — kies de route die bij jouw werk past.

---

## De controles

### 7. Je eerste check: wordt hier gewoond?

<!-- ▼ VIDEOBLOK — vervang FILMPJE-7 door de YouTube-ID ▼ -->

[![Filmpje 7 — Wordt hier gewoond?](https://img.youtube.com/vi/FILMPJE-7/hqdefault.jpg)](https://www.youtube.com/watch?v=FILMPJE-7)

<!-- GitHub Pages-variant:
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/FILMPJE-7" title="Filmpje 7 — Wordt hier gewoond?" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
-->

<!-- ▲ EINDE VIDEOBLOK ▲ -->

Wordt hier gewoond of niet? We lezen het rechtstreeks uit het model af, via de ruimtes — zonder één tekening open te slaan.

**2 à 3 minuten** · Je eerste echte controle.

---

### 8. Binnenwand of buitenwand?

<!-- ▼ VIDEOBLOK — vervang FILMPJE-8 door de YouTube-ID ▼ -->

[![Filmpje 8 — Binnenwand of buitenwand?](https://img.youtube.com/vi/FILMPJE-8/hqdefault.jpg)](https://www.youtube.com/watch?v=FILMPJE-8)

<!-- GitHub Pages-variant:
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/FILMPJE-8" title="Filmpje 8 — Binnenwand of buitenwand?" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
-->

<!-- ▲ EINDE VIDEOBLOK ▲ -->

De gevel bekijken zonder dat de binnenmuren in de weg zitten.

**2 à 3 minuten** · Bouwt voort op filmpje 4.

---

### 9. Staat het gebouw op de goede plek?

<!-- ▼ VIDEOBLOK — vervang FILMPJE-9 door de YouTube-ID ▼ -->

[![Filmpje 9 — Staat het gebouw op de goede plek?](https://img.youtube.com/vi/FILMPJE-9/hqdefault.jpg)](https://www.youtube.com/watch?v=FILMPJE-9)

<!-- GitHub Pages-variant:
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/FILMPJE-9" title="Filmpje 9 — Staat het gebouw op de goede plek?" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
-->

<!-- ▲ EINDE VIDEOBLOK ▲ -->

Staat het gebouw op het goede perceel en in de goede richting? En wat doe je als dat niet zo is?

**3 minuten** · Kijk eerst filmpje 5 en 6.

---

## Extra

### Wat zit er allemaal in een model?

<!-- ▼ VIDEOBLOK — vervang FILMPJE-STRUK door de YouTube-ID ▼ -->

[![Bonus — Wat zit er allemaal in een model?](https://img.youtube.com/vi/FILMPJE-STRUK/hqdefault.jpg)](https://www.youtube.com/watch?v=FILMPJE-STRUK)

<!-- GitHub Pages-variant:
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/FILMPJE-STRUK" title="Bonus — Wat zit er allemaal in een model?" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
-->

<!-- ▲ EINDE VIDEOBLOK ▲ -->

Kaartlagen, doorsnedes, materialen, classificaties, bouwlaaghoogtes — een rondje langs alles wat er in zo'n model blijkt te zitten.

**1 minuut**

---

<!--
NOG NIET GEPUBLICEERD — verwijder de HTML-commentaartekens rondom dit blok
zodra het IDS-filmpje is opgenomen.

### Voldoet dit model aan de informatie-eisen?

▼ VIDEOBLOK — vervang FILMPJE-IDS door de YouTube-ID ▼

[![Filmpje X — Voldoet dit model aan de informatie-eisen?](https://img.youtube.com/vi/FILMPJE-IDS/hqdefault.jpg)](https://www.youtube.com/watch?v=FILMPJE-IDS)

▲ EINDE VIDEOBLOK ▲

Niet of het gebouw goed is, maar of het bestand compleet is. Deze controle doe je vooraan, nog voor alle andere.

**3 minuten**
-->

---

## Meer weten?

- **Welke viewer kies ik?** Op onze site staat een lijst met BIM-viewers die je kunt gebruiken — een flink deel is gratis.
- **De hele reeks achter elkaar** staat als afspeellijst op ons YouTube-kanaal.
- **Liever een uur de tijd nemen?** Kijk dan het webinar; daarin gaan we dieper op het project in.
- **Vragen of iets niet duidelijk?** Laat het ons weten — deze filmpjes zijn er om gebruikt te worden, en we passen ze aan op wat je nodig hebt.
